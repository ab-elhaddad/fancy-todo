...
jest.mock('../../middlewares/authenticate.middleware.ts', () =>
...
jest.mock('../../middlewares/security/checkUserOfList.middleware.ts', () =>
  jest.fn((req, res, next) => next())
);
jest.mock('../../middlewares/security/checkUserOfTask.middleware.ts', () =>
  jest.fn((req, res, next) => next())
);
...
describe('Lists Controller', () => {
  ...
  describe('createList', () => {
    it('should create a new list', async () => {
      ...
      const response = await request(app).post('/lists').send(list);
  ...
  describe('getLists', () => {
    it('should return all lists for the authenticated user', async () => {
      const lists = [{ l_title: 'Test List 1' }, { l_title: 'Test List 2' }];

      // Mock the Lists.getAll method
      jest
        .mocked(Lists)
        .getAll.mockImplementation((l_user_id: number) =>
          Promise.resolve(lists as List[])
        );

      const response = await request(app).get('/lists');

      const listsResponse = response.body.lists.map((list: List) => {
        return { l_title: list.l_title };
      });
    ...
    it('should return an error if an error occurs while fetching the lists', async () => {
      ...
      jest.mocked(Lists).getAll.mockImplementation((userId: number) => {
        throw error;
      });
  ...
  describe('getList', () => {
    it('should return the requested list', async () => {
      ...
      jest
        .mocked(Lists)
        .get.mockImplementation((l_id: number) => Promise.resolve(list as List));

      const response = await request(app).get(`/lists/${list.l_id}`);
    ...
    it('should return an error if the list does not exist', async () => {
      ...
      jest.mocked(Lists).get.mockImplementation((l_id: number) => {
        throw error;
      });
  ...
  describe('deleteList', () => {
    it('should delete a list', async () => {
      ...
      const response = await request(app).delete('/lists').send(list);
    ...
    it('should return an error if the list does not exist', async () => {
      ...
      jest.mocked(Lists).delete.mockImplementation((l_id: number) => {
        throw error;
      });

      const response = await request(app).delete('/lists').send({ l_id: 1 });
  ...
  describe('updateList', () => {
    it('should update a list', async () => {
      ...
      const response = await request(app).put('/lists').send(list);
    ...
    it('should return an error if the list does not exist', async () => {
      ...
      jest.mocked(Lists).update.mockImplementation((list: List) => {
        throw error;
      });
  ...
  describe('shareList', () => {
    it('should generate a URL for the given list ID', async () => {
      ...
      jest
        .mocked(generateListURL)
        .mockImplementation((l_id: number) => 'http://localhost:3000/lists/token');

      const response = await request(app).get('/lists/share/1');

      expect(generateListURL).toHaveBeenCalledWith(1);
      expect(response.status).toBe(200);
      expect(response.body).toEqual({
        message: 'URL generated successfully.',
        url: 'http://localhost:3000/lists/token'
      });
    });

    it('should call the next middleware with an error if an error occurs', async () => {
      // Mock the generateListURL method
      jest.mocked(generateListURL).mockImplementation((l_id: number) => {
        throw new Error();
      });
