...
describe('Lists', () => {
	...
	beforeAll(async () => {
		...
		userId2 = (await prisma.user.create({
			data: {
				u_name: 'Test User 2',
	...
	describe('addTask', () => {
		it('should add a task to a list', async () => {
			...
			const taskList = (await prisma.$queryRaw`
				SELECT * FROM public."Task_List"
				WHERE tl_list_id = ${createdList.l_id}
				AND tl_task_id = ${createdTask.t_id}` as any[])[0]; // eslint-disable-line @typescript-eslint/no-explicit-any
