  ...
  static resetPassword = express
    ...
    .use((req: Request, res: Response, next: NextFunction) => {
      ...
      if (bodyError || paramsError)
        res.status(400).json({
          message: (bodyError ? bodyError : paramsError)?.details[0].message
        });
      else next();
    });

  static confirm = express
    .Router()
    .use((req: Request, res: Response, next: NextFunction) => {
      const schema = joi.object({
        token: joi.string()
